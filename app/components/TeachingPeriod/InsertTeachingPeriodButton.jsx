...
export default function InsertTeachingPeriodButton({ semesterString, insert, appendSemester, mobile, noFloat, bottom }) {
    ...
    return (
        <Button.Group fluid={mobile} className={"no-print" + (mobile || noFloat ? "" : " right floated")}>
            <Popup
              trigger={<Button className="btnorange" onClick={appendSemester}><Icon name="add square"/>Add {semesterString}</Button>}
            ...
            <Dropdown floating button className="icon btnorange" icon={bottom ? "caret up" : "caret down"} label="Add Custom Semester">
                <Dropdown.Menu style={bottom ? {top: "auto", bottom: "120%"} : {}} className="btnorange">
